...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!eventType) return json(400, { ok: false, error: "Missing event_type" });
    ...
    const { error } = await db.from("fl4sh_lite_events").insert({
      user_id: userId,
      event_type: eventType,
      placement: payload.placement ? String(payload.placement) : null,
